...
var safeStringify = _interopDefault(require('json-stringify-safe'));
...
var safeStringParams = [null, null, R.always('~~~ Circular Reference ~~~')];

// create a function which decircularizes into a string
var decircularizeToString = R.partialRight(safeStringify, safeStringParams);

// takes an object and returns an object that has be decircularized
var decircularize = R.pipe(decircularizeToString, JSON.parse);

// given a function, return a transformed name
var getFunctionName = function getFunctionName(fn) {
  return !RS.isNilOrEmpty(fn.name) ? '~~~ ' + fn.name + '() ~~~' : '~~~ Anonymous Function ~~~';
};

// replaces any function values found along the way with their names
var replaceValuesWithSomethingWeCanSeeOnTheOtherSideOfSerialization = function replaceValuesWithSomethingWeCanSeeOnTheOtherSideOfSerialization(originalObject) {
  var visited = [];

  function traverse(object) {
    // have we been here before?
    if (R.contains(object, visited)) return;

    // remember that we've been here
    if (R.is(Object, object)) {
      visited.push(object);
    }

    // loop through keys
    for (var key in object) {
      if (!R.has(key)) continue; // jet unless this key is our own
      ...
      switch (type) {
        case 'object':
          traverse(value);break;
        case 'array':
          R.apply(traverse, value);break;
        case 'function':
          // mutate this sucker!
...
var scrub = R.pipe(R.clone, replaceValuesWithSomethingWeCanSeeOnTheOtherSideOfSerialization, decircularize);
...
var Client = function () {
  ...
  createClass(Client, [{
    ...
    value: function send(type) {
      ...
      var actualPayload = this.options.safeRecursion ? scrub(payload) : payload;
